...
def create_character(level: int, archetype: str, *, class_name: Optional[str] = None,
    ...
    if class_name is None:
        ...
        if subclass_name is not None:
            # Both class and subclass provided; respect the explicit choice
            subclass_info = SUBCLASSES[cls][subclass_name]
        else:
            # Only class provided: pick a subclass based on archetype preference if available
            ...
            if pref_name and pref_name in SUBCLASSES[cls]:
                ...
                # No preference (e.g. Random archetype) or unknown mapping: choose a subclass at random
                available = list(SUBCLASSES[cls].keys())
                subclass_choice = random.choice(available)
                subclass_info = SUBCLASSES[cls][subclass_choice]
...
ANCESTRIES: Dict[str, Tuple[str, str]] = {
    # Ancestry features follow the Daggerheart Creation Package (SRD).  Each
    # ancestry grants two distinct abilities at character creation【373724897915597†L2043-L2064】.
    'Clank': (
        'Purposeful Design: Choose an Experience that aligns with your purpose; gain a permanent +1 bonus to it.',
    ...
    'Drakona': (
        ...
        'Elemental Breath: Choose an element for your breath; use it as an Instinct weapon within Very Close range (d8 magic damage).'
    ),
    'Dwarf': (
    ...
    'Elf': (
        ...
        'Celestial Trance: During a rest, drop into a trance to choose an additional downtime move.'
    ),
    'Faerie': (
        'Luckbender: Once per session, after you or an ally within Close range makes an action roll, spend 3 Hope to reroll the Duality Dice.',
        'Wings: You can fly; while flying you may mark a Stress after an attack against you to gain +2 Evasion against that attack.'
    ),
    'Faun': (
        'Caprine Leap: You can leap anywhere within Close range as though you were using normal movement.',
        'Kick: After a successful attack within Melee range, mark a Stress to deal 2d6 extra damage and either knock yourself or the target back to Very Close range.'
    ),
    'Firbolg': (
        'Charge: When you succeed on an Agility Roll to move from Far or Very Far into Melee range with targets, mark a Stress to deal 1d12 physical damage to all targets within Melee.',
        'Unshakable: When you would mark a Stress, roll a d6; on a 6, don’t mark it.'
    ),
    'Fungril': (
        'Fungril Network: Make an Instinct Roll (difficulty 12) to use your mycelial array to communicate with others of your ancestry across any distance.',
        'Death Connection: While touching a recent corpse, mark a Stress to extract one memory from it related to a specific emotion or sensation of your choice.'
    ),
    'Galapa': (
        'Shell: Gain a bonus to your damage thresholds equal to your Proficiency.',
        'Retract: Mark a Stress to retract into your shell; while retracted you have resistance to physical damage, disadvantage on action rolls, and cannot move.'
    ),
    'Giant': (
        'Endurance: Gain an additional Hit Point slot at character creation.',
        'Reach: Treat any weapon, ability, spell, or other feature that has a Melee range as though it has Very Close range.'
    ),
    'Goblin': (
    ...
    'Orc': (
        ...
        'Tusks: After you succeed on an attack within Melee range, you can spend a Hope to gore the target with your tusks, dealing an extra 1d6 damage.'
    ),
    'Ribbet': (
        'Amphibious: You can breathe and move naturally underwater.',
        'Long Tongue: You can use your long tongue to grab objects within Close range.  Mark a Stress to use it as a Finesse Close weapon that deals d12 physical damage using your Proficiency.'
    ),
    'Simiah': (
...
COMMUNITIES: Dict[str, str] = {
    # Community features reflect cultural upbringing and grant unique advantages【373724897915597†L2274-L2369】.
    ...
    'Loreborne': 'Well‑Read: You have advantage on rolls involving the history, culture or politics of a prominent person or place.',
    'Orderborne': 'Dedicated: Record three sayings or values your upbringing instilled in you. Once per rest, when you describe how you embody one of these principles, you can roll a d20 as your Hope Die.',
    ...
    'Seaborne': 'Know the Tide: When you roll with Fear, place a token on your community card (max tokens equal to your level). Before an action roll, spend any number of these tokens to gain a +1 bonus per token. Clear all unspent tokens at the end of the session.',
    'Slyborne': 'Scoundrel: You have advantage on rolls to negotiate with criminals, detect lies, or find a safe place to hide.',
    'Underborne': 'Low‑Light Living: In areas with low light or heavy shadow, you have advantage on rolls to hide, investigate, or perceive details within that area.',
    'Wanderborne': 'Nomadic Pack: Add a Nomadic Pack to your inventory. Once per session, spend a Hope to pull out a useful mundane item from the pack.',
